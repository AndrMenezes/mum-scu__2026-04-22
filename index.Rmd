...
- Posterior sampling of $\{(\mathcal{T}_h, \mathcal{M}_h)\}_{h=1}^m$.
  - Markov chain Monte Carlo (MCMC) methods with the Bayesian backfitting algorithm of `r Citet(bib, "Hastie2000")`.
...
\begin{cases}
\operatorname{TN}_{\lbrack -\infty, 0\rbrack}\lbrack f_j^{(0)}(\mathbf{x}_i), 1 \rbrack, & \textrm{if} \: z_{ij} = 1,\\
\operatorname{TN}_{\lbrack 0, \infty\rbrack}\lbrack f_j^{(0)}(\mathbf{x}_i), 1 \rbrack, & \textrm{if} \: z_{ij} = 0.
\end{cases}$$
--
