...
- Applications:
  ...
  - High-throughput sequencing experiments in biology, including microbiome
   `r Citep(bib, "Fernandes2014")` and single-cell `r Citep(bib, "Buttner2021")` experiments.

---
## The underlying probability model

- Experiment: $N$ **independent** trials. Each trial results in one of the $d$ mutually exclusive categories, where $N = \sum_{j=1}^d Y_j$ is known.

- Let $E_j := ``\textrm{category } j \textrm{ occurs''}$ with probability $\Pr\lbrack E_j\rbrack = \theta_j \ge 0$ for $j\in\{1, \ldots,d\}$. Note that $\Pr\left\lbrack \bigcup_{j=1}^d E_j\right\rbrack = \sum_{j=1}^d \Pr\lbrack E_j \rbrack = \sum_{j=1}^d \theta_j = 1$.
...
---
## Pollen data: Location

```{r map-location, out.width="100%", fig.align="center"}
# Load map data
world_map <- map_data("world")
data_pollen_wide$total_zeros <- rowSums(data_pollen_wide[, sp] == 0)
# data_pollen_wide$mean <- rowMeans(data_pollen_wide[, sp])
p_map <- ggplot() +
  geom_polygon(data = world_map[world_map$lat > 20, ],
               aes(x = long, y = lat, group = group),
               fill = "grey", alpha = 0.4, col = "black") + 
  geom_point(data = data_pollen_wide,
             mapping = aes(x = longitude, y = latitude, col = total_zeros),
             size = 0.6) +
  coord_fixed(1.6) +
  colorspace::scale_color_continuous_sequential(palette = "BluYl", rev = TRUE) +
  # colorspace::scale_color_binned_divergingx(palette = "Spectral", 
  #                                           breaks = c(0, 10, 20, 28)) +
  theme(legend.text = element_text(size = 10),
        legend.key.width = unit(1, 'cm')) +
  scale_x_continuous(breaks = scales::pretty_breaks(6)) +
  labs(x = "Longitude", y = "Latitude", col = "") +
  ggtitle(label = "Location of the sediment cores",
          subtitle = "Number of species (out of 28) with zero count")
p_map



# plot_grid(p_map, p_mean_total)
...
- `r Citet(bib, "Menezes2025")` derived the ZANIM distribution to account for structural zeros
across the categories of the multinomial distribution.
...
- Posterior sampling of $\{(\mathcal{T}_h, \mathcal{M}_h)\}_{h=1}^m$: similar procedure, but using a *generalised* (modified) Bayesian backfitting.
...
- All models included in the comparison are implemented in our `R` package `zanicc`, available at
